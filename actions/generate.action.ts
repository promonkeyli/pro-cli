...
const generateFiles = async (inputs: Input[]) => {
    ...
    const collectionOption = inputs.find((option) => option.name === "collection")!.value as string;
    const schematic = inputs.find((option) => option.name === "schematic")!.value as string;
    ...
    const collection: AbstractCollection = CollectionFactory.create(
        collectionOption || configuration.collection || Collection.PRO,
    );
    ...
    let generateSpec = shouldGenerateSpec(configuration, schematic, appName, specValue, specOptions.passedAsInput);
